...
def extension_info(result):
    """Get toplevel dir, extension_slug, and project dir from baked cookies"""
    extension_path = str(result.project)
    extension_slug = os.path.split(extension_path)[-1]
    extension_dir = os.path.join(extension_path, extension_slug)
    return extension_path, extension_slug, extension_dir
...
def test_bake_and_run_travis_pypi_setup(cookies):
    ...
    with bake_in_temp_dir(cookies) as result:
        ...
        run_inside_dir(travis_setup_cmd, extension_path)
        # then:
...
def test_extension_with_hyphen_in_module_name(cookies):
    result = cookies.bake(extra_context={'extension_name': 'something-with-a-dash'})
    ...
    # then:
    result_travis_config = yaml.load(open(os.path.join(extension_path, ".travis.yml")))
...
def test_bake_with_no_console_script(cookies):
    ...
    extension_path, extension_slug, extension_dir = extension_info(result)
    found_extension_files = os.listdir(extension_dir)
    assert "cli.py" not in found_extension_files

    setup_path = os.path.join(extension_path, 'setup.py')
...
def test_bake_with_console_script_files(cookies):
    ...
    extension_path, extension_slug, extension_dir = extension_info(result)
    found_extension_files = os.listdir(extension_dir)
    assert "cli.py" in found_extension_files
...
def test_bake_with_console_script_cli(cookies):
    ...
    result = cookies.bake(extra_context=context)
    extension_path, extension_slug, extension_dir = extension_info(result)
    module_path = os.path.join(extension_dir, 'cli.py')
    module_name = '.'.join([extension_slug, 'cli'])
    ...
    noarg_output = ' '.join(['Replace this message by putting your code into', extension_slug])
